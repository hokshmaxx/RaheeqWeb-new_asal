...
@section('title') {{ucwords(__('cp.ages'))}}
@endsection
@section('css')
...
{{--                        </button>--}}
                    ...
                    <a href="{{url(getLocal().'/admin/ages/create')}}" class="btn btn-secondary  mr-2 btn-success" data-target="#basicModal" data-toggle="modal">
                    ...
                    <div class="contentTabel">
                        ...
                        <div class="table-responsive">
                            ...
                            <table class="table table-bordered table-hover table-checkable"  style="margin-top: 13px !important" id="kt_datatable1">
                               <thead>
                                <tr>
                                    <th class="wd-1p no-sort" style="width: 1px;">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="add-employee-form">
                        <form class="create_form" method="post" action="{{url(app()->getLocale().'/admin/ages')}}" enctype="multipart/form-data">
                            @csrf
                            <div class="row">
...
$(document).on('click','.create_send_form',function (e) {
            ...
            $(this).closest(".create_form").find( 'select, textarea, input' ).each(function(){
      ...
      // $('.contact_us').html('<i class="fa fa-spinner fa-spin" style="font-size: 20px;"></i>'+' '+'{{__('website.send')}}');
        ...
        var id = $(this).data("id");
        $.ajax({
            url: '{{url(app()->getLocale().'/admin/ages')}}', 
            type: "post", 
            data: formData,
            processData: false,
            contentType: false,
...
$(document).on('click','.edit_item_btn',function (e) {
       ...
       var id = $(this).data("id");
        $.ajax({
            url: '{{url(app()->getLocale().'/admin/ages')}}'+'/'+id+'/edit', 
            type: "get", 
            success: function (response) {
            // return response;
                if(response.code ==300){
    ...
    $(document).on('click','.edit_send_form',function (e) {
            ...
            $(this).closest(".edit_form").find( 'select, textarea, input' ).each(function(){
                  if($(this).prop('required') && !$(this).val() && !$(this).is(":hidden")){
                      $(this).css("border", "#ff0000 solid 1px").next('span.errorSpan').remove(); //
        ...
        $.ajax({
            ...
            url: '{{url(app()->getLocale().'/admin/ages')}}'+'/'+item_id, 
            type: "post", 
            data: editFormData,
            processData: false,
            contentType: false,
    ...
    function delete_adv(id, iss_id, e) {
            ...
            var url = '{{url(getLocal()."/admin/ages")}}/' + id;
