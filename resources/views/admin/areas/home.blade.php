...
@section('title') {{ucwords(__('cp.areas'))}}
@endsection
@section('css')
...
{{--                        </button>--}}
                    ...
                    <a href="{{url(getLocal().'/admin/areas/create')}}" class="btn btn-secondary  mr-2 btn-success" data-target="#basicModal" data-toggle="modal">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="add-employee-form">
                        <form class="create_form" method="post" action="{{url(app()->getLocale().'/admin/areas')}}" enctype="multipart/form-data">
                            @csrf
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group ">
                                        ...
                                        <input onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,'')" type="text"
                                         class="form-control form-control-solid" name="delivery_cost" value="{{ old('delivery_cost')}}" required/>
                                    </div>
                               </div>
                            </div>
                        </form>
...
$(document).on('click','.create_send_form',function (e) {
      ...
      // $('.contact_us').html('<i class="fa fa-spinner fa-spin" style="font-size: 20px;"></i>'+' '+'{{__('website.send')}}');
        ...
        var id = $(this).data("id");
        $.ajax({
            url: '{{url(app()->getLocale().'/admin/areas')}}', 
            type: "post", 
            data: formData,
            processData: false,
            contentType: false,
...
$(document).on('click','.edit_item_btn',function (e) {
       ...
       var id = $(this).data("id");
        $.ajax({
            url: '{{url(app()->getLocale().'/admin/areas')}}'+'/'+id+'/edit', 
            type: "get", 
            success: function (response) {
            // return response;
                if(response.code ==300){
    ...
    $(document).on('click','.edit_send_form',function (e) {
        ...
        $.ajax({
            ...
            url: '{{url(app()->getLocale().'/admin/areas')}}'+'/'+item_id, 
            type: "post", 
            data: editFormData,
            processData: false,
            contentType: false,
    ...
    function delete_adv(id, iss_id, e) {
            ...
            var url = '{{url(getLocal()."/admin/areas")}}/' + id;
